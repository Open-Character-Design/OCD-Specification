...
The `@ocs-tools/validator` package delivers the Open Character Specification validator, parser, and command line tooling for Node.js consumers.
...
# or
pnpm add @ocs-tools/validator
# or
yarn add @ocs-tools/validator
```
...
The package exposes the `ocs-validate` executable. Use it directly or through `npx`:

```bash
# Validate a document and print the normalized output
npx @ocs-tools/validator --print examples/bruenor.yaml

# Treat warnings as build failures
npx @ocs-tools/validator --warnings-as-errors examples/bruenor.yaml
```
...
The package also exports the generated OCS-T parser via `@ocs-tools/validator/parser`.
